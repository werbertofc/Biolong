...
def eat_to_access_token(eat_token: str) -> Dict[str, Any]:
    try:
        ...
        return {
            ...
            'message': 'Unable to convert EAT token. Token may be invalid or expired.'
        }
    except Exception as e:
        return {
            'success': False,
...
def generate_jwt_token(uid, password):
    ...
    return major_login(access_token, open_id)
...
def generate_jwt_from_eat_token(eat_token):
    eat_result = eat_to_access_token(eat_token)
    if not eat_result or not eat_result.get('success'):
        return None
    access_token = eat_result['access_token']
    open_id = eat_result['open_id']
    platform_type = eat_result['platform_type']
    return major_login(access_token, open_id, platform_type)
...
def get_jwt_from_eat_token(eat_token):
    cache_key = f"eat_{eat_token}"
    token_data = jwt_token_cache.get(cache_key)
    if token_data and token_data['expiry'] > datetime.utcnow():
        return token_data['token']
    token = generate_jwt_from_eat_token(eat_token)
    if token:
        expiry_time = datetime.utcnow() + timedelta(hours=JWT_LIFETIME_HOURS)
        jwt_token_cache[cache_key] = {'token': token, 'expiry': expiry_time}
        return token
...
def home():
    return jsonify({
        ...
        "endpoints": {
            "/update_bio": "POST (form/data/json) with bio and one of: token, uid+password, access_token, eat_token",
            "/api/update_bio": "same as /update_bio"
        },
...
def update_bio():
    try:
        ...
        eat_token = data.get('eat_token') or request.args.get('eat_token') or request.form.get('eat_token')
        ...
        if len(lines) > 3:
            return jsonify({
                "success": False,
                "error": "Bio must have maximum 3 lines",
                "credit": "t.me/danger_ff_like"
            }), 400
        ...
        if token:
            ...
            if not jwt_token:
                return jsonify({
                    "success": False,
                    "error": "Failed to generate JWT token. Please check your UID and password.",
                    "credit": "t.me/danger_ff_like"
                }), 400
        elif access_token:
            ...
            if not jwt_token:
                return jsonify({
                    "success": False,
                    "error": "Failed to generate JWT token from access token. Please check your access token.",
                    "credit": "t.me/danger_ff_like"
                }), 400
        elif eat_token:
            jwt_token = get_jwt_from_eat_token(eat_token)
            if not jwt_token:
                return jsonify({
                    "success": False,
                    "error": "Failed to generate JWT token from EAT token. Please check your EAT token.",
                    "credit": "t.me/danger_ff_like"
                }), 400
        else:
            return jsonify({
                "success": False,
                "error": "Authentication method is required (token, uid+password, access_token, or eat_token)",
                "credit": "t.me/danger_ff_like"
            }), 400
